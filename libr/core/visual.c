...
R_API void r_core_visual_title(RCore *core, int color) {
	...
	}
	{
		...
		if (__ime) {
			...
			if (core->print->cur_enabled) {
				if (core->print->ocur == -1) {
					title = r_str_newf ("[%s *0x%08"PFMT64x" ($$+0x%x)]> %s %s\n",
						address, core->offset + core->print->cur,
						core->print->cur,
						bar, pos);
				} else {
					title = r_str_newf ("[%s 0x%08"PFMT64x" [0x%x..0x%x] %d]> %s %s\n",
						address, core->offset + core->print->cur,
						core->print->ocur, core->print->cur, R_ABS (core->print->cur - core->print->ocur) + 1,
						bar, pos);
				}
			} else {
				title = r_str_newf ("[%s %s%d %s]> %s %s\n",
					address, pcs, core->blocksize, filename, bar, pos);
			}
		}
